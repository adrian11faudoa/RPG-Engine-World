...
# ─── Green Target Group (used by CodeDeploy Blue/Green) ────────
# Identical config to blue — CodeDeploy swaps ALB traffic between them.
resource "aws_lb_target_group" "dashboard_green" {
  name        = "${var.name_prefix}-tg-dashboard-green"
  port        = 3000
  protocol    = "HTTP"
  vpc_id      = var.vpc_id
  target_type = "ip"
